...
def hexlist_to_dec(x_list):
    ...
    y = "0x"+"".join(x_list)
    return int(y,16)
def mutliHexString_to_dec(hexString, startingIdx = 36,dataStartIdx = 2,dataEndIdx = 6):
    x_list = hexString[36:-1].split(" ")
...
def calculate_temp(df):
    ...
    for i,row in df.iterrows():
        if count==0:
            pass
        count = count +1
        dec_values_list.append(mutliHexString_to_dec(row.iloc[0],dataStartIdx=6,dataEndIdx=8)/10)
    return dec_values_list
